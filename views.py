...
def dossier(id, without_changes=True):
    ...
    if 'legal_basis' in dossier.get('procedure', {}):
        clean_lb(dossier)
    if 'changes' in dossier and without_changes:
        del dossier['changes']
    for act in dossier['activities']:
        if act.get('type') in [
            'Non-legislative initial document',
    ...
    for vote in votes:
        ...
        if t and r:
            ...
            if i >= 0:
                ...
                vote['linkedtitle'] = '%s<a href="http://www.europarl.europa.eu/sides/getDoc.do?type=REPORT&amp;mode=XML&amp;reference=%s&amp;language=EN">%s</a>%s' % (t[:i], rid, r, t[i + len(r):])
    ...
    for item in db.ep_comagendas.find({'epdoc': dossier['procedure']['reference']}):
        ...
        if 'tabling_deadline' in item and item['tabling_deadline'] >= datetime.now():
            deadline = {
                'type': '(%s) Tabling Deadline' % item['committee'],
                'body': 'EP',
...
def committee(id):
    # get agendas
    agendas = []
    for entry in db.ep_comagendas.find({
        'committee': id,
        'date': {'$exists': True}
    }).sort([
    ...
    for d in db.dossiers2.find({'activities.committees.committee': id, 'procedure.stage_reached': {'$in': STAGES}}):
        ...
        if len(tmp) > 0:
            ...
            for act in d['activities']:
                ...
                ] and 'docs' in act and len(act['docs']) > 0:
    ...
    for mep in db.ep_meps2.find(query):
        for c in mep['Committees']:
            if c['abbr'] == id:
                ...
                break
    return {
        'meps': [x for _, x in sorted(rankedMeps, reverse=True)],
        'dossiers': dossiers,
        'agendas': agendas
    }
...
def tonewmep(oid):
    ...
    return (db.ep_meps2.find_one({'UserID': int(omep['UserID'])}, []) or db.ep_meps2.find_one({'UserID': str(omep['UserID'])}, []))['_id']
...
def subjects():
    ...
    for d in db.dossiers2.find():
        ...
        if not len(subs):
            continue
        buck = []
        for actor, committee in [(a, committee) for committee in d['committees'] for a in committee.get('rapporteur', []) if hasattr(a, 'keys') and committee.get('responsible') and a.get('mepref')]:
            if actor in buck:
                continue
            buck.append(actor)
            if type(committee.get('date')) == type(list()):
                (country, party) = getCountry(fullmeps.get(actor['mepref']) or fullmeps[tonewmep(actor['mepref'])], committee.get('date')[committee['rapporteur'].index(actor)])
            else:
                (country, party) = getCountry(fullmeps.get(actor['mepref']) or fullmeps[tonewmep(actor['mepref'])], committee.get('date'))
    ...
    csv = [(count, '.'.join([str(x) for x in subj]), k[0], subjectscache[subj]['title'][0], k[1], k[2]) for k, v in all.items() for subj, count in v.items() if subj != 'total']
    return (csv, tree)
    # print u'\n'.join([u'\t'.join([unicode(y) for y in x]) for x in sorted(csv,reverse=True)]).encode('utf8')

def amendment(seq, committee, dossier):
    res = db.ep_ams.find_one({
        'seq': seq,
        'committee': committee,
        'reference': dossier
    })
